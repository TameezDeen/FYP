...
const { filterSongsByGenres, saveFilteredSongsToDatabase } = require('../controllers/songController'); // Import the function
...
router.post('/filtered-songs', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Save filtered songs to the database
    await saveFilteredSongsToDatabase(filteredSongs, languagePreference);
